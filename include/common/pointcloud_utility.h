...
PclPointToEigen<float, 3, relocalization::PointXYZIRT>(
    const relocalization::PointXYZIRT& pt) {
    return pt.getVector3fMap();
}
...
inline relocalization::PointXYZIRT TransformPoint(
    const relocalization::PointXYZIRT& pt, const Mat3f& R, const Vec3f& t) {
    const Vec3f temp = R * Vec3f(pt.x, pt.y, pt.z) + t;
    relocalization::PointXYZIRT pt_trans{};
    pt_trans.x = temp.x();
    pt_trans.y = temp.y();
    pt_trans.z = temp.z();
    pt_trans.intensity = pt.intensity;
